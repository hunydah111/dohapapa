...
//   stability 🐢 꾸준왕(안정)  : snapshot 평형 변동성 평균 낮을수록 상위
//
...
interface Complex { sigungu: string; medians: Median[] }
...
// ── 2) snapshot 집계: 거래수·평단가(만원/평)·변동성 ──
const agg: Record<string, { trades: number; complexCount: number; perPy: number[]; vol: number[] }> = {};
for (const c of complexes) {
  if (!c.sigungu || !c.medians?.length) continue;
  const a = (agg[c.sigungu] = agg[c.sigungu] || { trades: 0, complexCount: 0, perPy: [], vol: [] });
  // 대표 평형 = 거래수 최대
  const rep = c.medians.reduce((m, x) => (x.count > m.count ? x : m), c.medians[0]);
  let cTrades = 0;
  for (const m of c.medians) {
    cTrades += m.count || 0;
    if (m.volatility != null && m.count >= 2) a.vol.push(m.volatility);
  }
  a.trades += cTrades;
  a.complexCount += 1;
...
const rStab = rankBy(withValue, (m) => m.volatility, false); // 낮을수록 상위
...
const regions = withValue.map((m) => ({
  ...
  volatility: Math.round(m.volatility * 1000) / 1000,
  ranks: {
    momentum: rMom.get(m)!,
    trades: rTrades.get(m)!,
    value: rValue.get(m)!,
    stability: rStab.get(m)!,
  },
}));
...
const board = (label: string, key: "momentum" | "trades" | "value" | "stability") =>
  console.log(`\n[${label}] ` + regions
    .slice()
    .sort((p, q) => p.ranks[key] - q.ranks[key])
    .slice(0, 5)
    .map((r) => `${r.sigungu}(${key === "momentum" ? r.momentumPct + "%" : key === "trades" ? r.trades + "건" : key === "value" ? r.pricePerPy + "만/평" : "변" + r.volatility})`)
    .join(" · "));
