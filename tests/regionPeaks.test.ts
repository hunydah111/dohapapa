...
describe("computeRegionPeaks — 회복률·신고점 돌파", () => {
  ...
  it("현재월 표본이 전고점 문턱(10) 미달이면서 더 높으면 신고점 돌파 recovery > 1", () => {
    // 전고점 후보는 ≥10건 달뿐. 현재월(5건)은 후보 아님 → 전고점=과거 20억, 현재 24억 → 1.2.
    const months = ["2021-01", "2021-02", "2021-03"];
    const rows = [
      ...deals("서초구", "2021-01", 2_000_000_000, 10),
      ...deals("서초구", "2021-02", 2_000_000_000, 10),
      ...deals("서초구", "2021-03", 2_400_000_000, 5), // 현재 — 소표본이라 후보 아님
    ];
    const out = computeRegionPeaks(rows, months);
    expect(out["서초구"].peakKrw).toBe(2_000_000_000);
    expect(out["서초구"].currentKrw).toBe(2_400_000_000);
    expect(out["서초구"].recovery).toBeCloseTo(1.2, 10);
  });
});
