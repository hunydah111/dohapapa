# 04 — 동네 자존심 리그 비지 2종 (League)

**용도:** /league "동네 자존심 리그" 화면. ① **flag** = 헤더 히어로(깃발 든 비지, 응원·자존심 톤) ② **champ** = 1위 동네에 뜨는 왕관 챔피언 비지(보드 1위 행 + 내 동네가 1위일 때 자랑 카드 상단). 둘 다 비버(비지) 하나에 **소품·표정만** 얹음(다른 캐릭터 X).
