...
export const POLICY_META = {
  ...
  /** 은행 자체 재원 주담대 최근 동향 경고 — 정부 cap(수도권 규제 6억)과 별개로 은행들이
   *  자체 한도를 축소 중. KB국민이 2026-07-10부터 수도권·규제지역 주택구입자금(생애최초
   *  포함)을 6억→3억으로 절반 축소. 은행마다 달라 앱은 정부 기준(6억)으로 추정하되 이 경고를
   *  병기한다(3억 일괄 적용 시 타행 이용자 과소추정). 정책자금(디딤돌·보금자리)은 제외 대상. */
  bankTighteningNote:
    "일부 은행은 자체적으로 주담대 한도를 축소 중입니다(예: KB국민 2026-07-10부터 수도권·규제지역 생애최초 포함 6억→3억). 앱은 정부 기준(수도권 규제 6억)으로 추정하므로, 실제 은행 한도는 이보다 낮을 수 있습니다. 정책자금(디딤돌·보금자리)은 이 축소와 별개입니다.",
} as const;
