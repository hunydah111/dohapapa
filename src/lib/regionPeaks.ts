...
// 아니며, 라벨·각주에 반드시 "국민평형(전용 84㎡급) 실거래 중위 기준 — 시세 지수 아님"을
// 병기한다.
//
// ★밴드 고정(국민평형 84㎡급 = p32_35)★ — 호출부(build-region-series)가 국평 거래만 걸러
// 넘긴다. 시군구 "평형 혼합" 중위로 절대가 전고점을 잡으면 특정 달 대형·고가 쏠림이 가짜
// 전고점을 만든다(2026-07-07 실측: 78곳 중 42곳 전고점이 2026년으로 잡히고 송파 "한 달 만에
// −38%" 오보). 국평 한 밴드로 고정하면 평형 구성 왜곡이 원천 제거된다 — computeRegionPeaks
// 자체는 밴드를 모르고 월 중위만 계산하므로, 각주엔 "국민평형 기준 — 다른 평형은 회복 양상이
// 다를 수 있음"을 병기한다.
//
