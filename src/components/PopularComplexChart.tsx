...
// 멜론식 "이번 주 인기 아파트" 차트 — 비집고에서 조건에 맞아 많이 뜬 수도권 단지의 주간 순위.
// 비-PII 집계(/api/popular-complexes). 표본이 빈약하면 조용히 숨는다(동네 차트와 같은 철학).
// 컴플라이언스: 사용집계(사실)일 뿐 추천·광고·순위보장 아님 — 하단 면책 명시.
...
// 표본이 너무 적으면 숨김 — 빈약한 차트의 신뢰도 문제 방지.
const MIN_ENTRIES = 5;
const MIN_TOTAL = 10;
...
export function PopularComplexChart() {
  const [data, setData] = useState<ChartData | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetch("/api/popular-complexes")
  ...
  return (
    <section className="mx-auto mt-6 w-full max-w-sm text-left">
      ...
      >
        <ul className="flex flex-col gap-2.5">
          {data.entries.map((e) => {
            const top = e.rank <= 3;
            return (
              <li key={`${e.complexName}-${e.dongName}`} className="flex items-center gap-3">
                <span
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-[11px]" style={{ color: "#9a8f82" }}>
                    {e.sigungu} · {e.dongName}
                  </p>
        ...
        <p className="mt-3 text-[11px] leading-relaxed" style={{ color: "#9a8f82" }}>
          비집고에서 조건에 맞아 많이 뜬 수도권 단지 집계예요. 광고나 순위 보장이 아닙니다.
        </p>
      </div>
    </section>
  );
