...
import { getRegions, bestBoardFor, type Board, type LeagueRegion } from "@/lib/league";

// 오늘의 동네 — 날짜 시드로 매일 한 곳을 뽑아 "살아있음" 연출(DB0·번들). 배포 없이 매일 바뀜.
// 하이드레이션 안전: new Date()는 마운트 후(useEffect)에만 — 서버/클라 날짜 불일치 방지.
export function TodayNeighborhood({ onPick }: { onPick?: (sigungu: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1 leading-tight">
        ...
        <br />
        <span className="text-[15px] font-extrabold" style={{ color: "#3a2c1d" }}>{region.sigungu}</span>
        <span className="text-[13px] font-semibold" style={{ color: "#6e5b46" }}> — {board.emoji} {board.label} 전국 {rank}위</span>
      </span>
